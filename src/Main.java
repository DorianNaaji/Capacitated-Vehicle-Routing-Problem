...
public class Main extends Application
{
    ...
    private static Solution testRecuitItinéraire(Fichier fichier, int nbSolutionsInitiales, Génération typeGénération, int seuil, int températureInitiale, int nombreDeVoisinsParTempérature, double coefficientDeDiminutionTempérature, boolean isMétaTransformations, Transformation typeDeTransformation) throws Exception
    {
        ...
        for(Solution solutionCourante:solutionsAléatoires)
        {
            ...
            for (int j = 0; j < solutionCourante.getItinéraires().size(); j++) {
                ...
                Itinéraire itinéraireOptimisé = RecuitSimulé.recuitSimuléIt(solutionCourante.getItinéraires().get(j),
                        200,
                        100,
                        500,
                        0.95,
    ...
    private static Solution testTabou(Fichier fichier, int nbSolutionsInitiales, Génération typeGénération, int seuil, int tailleMaximaleListeTabou, int nbIterMax, int nbSolutionsVoisinesChaqueIter, Transformation typeDeTransformation, TypeDeRechercheVoisinage typeDeRechercheVoisinage) throws Exception
    {
        ...
        {
            // avec ces paramètres-ci, on trouve la meilleure solution sur le 1er fichier
            ...
            //System.out.println(solutionOptimisée.getOptimisationGlobale());
            if(solutionOptimisée.getOptimisationGlobale() < best.getOptimisationGlobale())
            {
